...
def test_chain2() -> None:
    ...
    assert Err('could not convert string to int') == \
           _string_to_int("asdas").flat_map(_safe_square_root) == \
           chained("asdas")

    assert Err("input not valid for square root") == \
...
def test_chain4() -> None:
    ...
    assert Err("must be two characters or greater") == \
           (_min_2_chars("9")
