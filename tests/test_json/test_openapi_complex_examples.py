...
def test_recursive_validator() -> None:
    ...
    comment_validator: v.Obj2[str, List[Comment], Comment] = v.Obj2(
        ...
        v.prop("replies", v.ArrayOf(v.Lazy(get_comment_recur))),
        into=Comment,
    )
...
def test_person() -> None:
    ...
    person_validator = v.Obj5(
        v.prop("name", v.String(v.not_blank)),
        v.prop("email", v.String(v.Email(), v.MaxLength(50))),
        v.prop(
            "occupation", v.String(v.Enum({"teacher", "engineer", "musician", "cook"}))
        ),
        v.prop("country_code", v.String(v.MinLength(2), v.MaxLength(3))),
        v.prop(
            "honorifics",
            v.ArrayOf(
                v.String(v.RegexValidator(re.compile(r"[A-Z][a-z]+\.]"))),
                v.unique_items,
                v.MaxItems(3),
            ),
        ),
